...
struct PieceCoordinates{
  ...
  PieceCoordinates( std::int32_t y = 0, std::int32_t x = 0 ):  y_( y ),
                                                             x_( x - 65 ) {}
};
...
public:
  ...
  const std::int32_t currentTurn() const{
    return turns_;
  }
...
public:
  ...
  void setPiece ( const Piece* p, std::int32_t x, std::int32_t y ){
...
protected:
  ...
  std::int32_t calcBattleBenefit( const PieceCoordinates& start,
                                 const PieceCoordinates& finish );
